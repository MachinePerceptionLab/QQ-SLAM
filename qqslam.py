...
##################################################################################
        self.tsdf_numpy = torch.load("tsdf_volume/tsdf_volume.pt").to(self.device).float()
        # self.tsdf_numpy = tsdf_disc(self.tsdf_numpy)
        self.tsdf_bounds = torch.from_numpy(torch.load("tsdf_volume/tsdf_bounds.pt")).to(self.device).float()
#################################################################################
...
##################################################################################
        ...
        with torch.no_grad():
            ############### init volume here ############### 
            self.tsdf_vol, self.intrinsic, _ = self.init_tsdf_volume(cfg, args)
            tsdf_volume, _, _ = self.tsdf_vol.get_volume()
            ...
            self.tsdf_volume = tsdf_volume.permute(0, 1, 4, 3, 2)
...
##################################################################################
    def init_tsdf_volume(self, cfg, args):
        ...
        # Maybe # enlarge the bound a bit to allow it divisible by bound_divisible
        bound_divisible = 0.32 #cfg['grid_len']['bound_divisible']
        bound[:, 1] = (((bound[:, 1]-bound[:, 0]) /
                        bound_divisible).int()+1)*bound_divisible+bound[:, 0]

        # TSDF volume
        H, W, fx, fy, cx, cy = self.update_cam(self.config)
    ...
    def fuse_gt(self):
        ...
        for idx, batch in tqdm(enumerate(data_loader)):
            ...
            start = -1
            trainskip = 4
            if (idx - start) % trainskip != 0:
                continue
            depth = batch['depth'].squeeze(0).cpu().numpy() # (368, 496, 3)
            color = batch['rgb'].squeeze(0).cpu().numpy()
            c2w_gt = batch['c2w'][0].to(self.device)

            c2w = c2w_gt.clone().detach().cpu().numpy()
            c2w[:3, 1] *= -1.0
            c2w[:3, 2] *= -1.0
            ...
            self.tsdf_vol.integrate(color, depth, self.intrinsic, c2w, obs_weight=1.)

        print("Saving mesh to mesh.ply...")
        verts, faces, norms, colors = self.tsdf_vol.get_mesh()
        fusion.meshwrite("tsdf_volume/office0_tsdf.ply", verts, faces, norms, colors)
        print('Getting TSDF volume')
        tsdf_volume, _ ,bounds = self.tsdf_vol.get_volume()
        tsdf_volume = torch.tensor(tsdf_volume)
        tsdf_volume = tsdf_volume.reshape(1, 1, tsdf_volume.shape[0], tsdf_volume.shape[1], tsdf_volume.shape[2])
        tsdf_volume = tsdf_volume.permute(0, 1, 4, 3, 2)
        torch.save(tsdf_volume, 'tsdf_volume/tsdf_volume.pt')
        torch.save(bounds, 'tsdf_volume/tsdf_bounds.pt')
    ...
    def get_loss_from_ret(self, ret, rgb=True, sdf=True, depth=True, fs=True, smooth=False):
        ...
        loss += self.config['training']['quant_weight'] * ret['diff']  # Commitment loss
        loss += self.config['training']['diversity_weight'] * ret['pairloss']  # Diversity loss
        return loss           

    def update_tsdf(self, batch, c2w):
        tsdf_c2w = c2w.clone().detach().cpu().numpy()
        tsdf_c2w[:3, 1] *= -1.0
        tsdf_c2w[:3, 2] *= -1.0
        depth = batch['depth'].squeeze(0).cpu().numpy().astype(np.float32)
        color = (batch['rgb'].squeeze(0).cpu().numpy() * 255).astype(np.uint8)
        self.tsdf_vol.integrate(color, depth, self.intrinsic, tsdf_c2w, obs_weight=1.)
        tsdf_volume, _, _ = self.tsdf_vol.get_volume()
        tsdf_volume = torch.tensor(tsdf_volume).float().to(self.device)
        tsdf_volume = tsdf_volume.reshape(1, 1, tsdf_volume.shape[0], tsdf_volume.shape[1], tsdf_volume.shape[2])
        self.tsdf_volume[:] = tsdf_volume.permute(0, 1, 4, 3, 2)
    ...
    def first_frame_mapping(self, batch, n_iters=100):
        ...
        if self.config["fuse_tsdf"]:
            ...
            self.tsdf_vol.integrate(color, depth, self.intrinsic, tsdf_c2w, obs_weight=1.)
            self.tsdf_vol_pred = copy.deepcopy(self.tsdf_vol)
            tsdf_volume, _, _ = self.tsdf_vol.get_volume()
            tsdf_volume = torch.tensor(tsdf_volume)
            tsdf_volume = tsdf_volume.reshape(1, 1, tsdf_volume.shape[0], tsdf_volume.shape[1], tsdf_volume.shape[2])
            tsdf_volume = tsdf_volume.permute(0, 1, 4, 3, 2)
        ...
        for i in range(n_iters):
            ...
            # Forward
            if self.config["fuse_tsdf"]:
                ret = self.model.forward(rays_o, rays_d, target_s, target_d,self.tsdf_volume,self.tsdf_bounds)
            else:
                ret = self.model.forward(rays_o, rays_d, target_s, target_d,self.tsdf_numpy,self.tsdf_bounds)
    ...
    def smoothness(self, sample_points=256, voxel_size=0.1, margin=0.05, color=False):
        ...
        # Forward
        if self.config["fuse_tsdf"]:
            sdf = self.model.query_sdf(pts_tcnn, embed=True,tsdf_numpy=self.tsdf_volume, tsdf_bounds=self.tsdf_bounds)[0]
        else:
            sdf = self.model.query_sdf(pts_tcnn, embed=True,tsdf_numpy=self.tsdf_numpy, tsdf_bounds=self.tsdf_bounds)[0]
    ...
    def save_mesh(self, i, voxel_size=0.05):
        ...
        if self.config["fuse_tsdf"]:
            extract_mesh(self.model.query_sdf, 
                            ...
                            mesh_savepath=mesh_savepath,tsdf_numpy=self.tsdf_volume, tsdf_bounds=self.tsdf_bounds)
        else:
            extract_mesh(self.model.query_sdf, 
                            self.config, 
                            self.bounding_box, 
                            ...
                            marching_cube_bound=self.marching_cube_bound, 
                            voxel_size=voxel_size, 
                            mesh_savepath=mesh_savepath,tsdf_numpy=self.tsdf_numpy, tsdf_bounds=self.tsdf_bounds)     
